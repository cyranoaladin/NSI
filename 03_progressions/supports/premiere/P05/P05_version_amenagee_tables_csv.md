---
title: "P05 - Version Amenagee - Tables CSV et requêtes simples"
level: "premiere"
sequence_id: "P05"
...
# P05 - Version Amenagee - Tables CSV et requêtes simples
...
- Situation : une association reçoit un fichier CSV de réservations et veut filtrer les lignes incohérentes.
- Donnée fournie : table structurée en lignes et colonnes.
- Exemple de départ : `nom;age;ville avec trois enregistrements`.
- Les étapes sont séparées pour réduire la charge de lecture.

## Aide intégrée
- Aide 1 : commence par recopier la donnée utile, ici table structurée en lignes et colonnes.
- Aide 2 : applique seulement cette méthode : lire l’en-tête, convertir les champs utiles, filtrer puis agréger.
- Aide 3 : compare avec le résultat de référence `liste de dictionnaires filtrée puis moyenne calculée`.
- Aide 4 : vérifie le cas limite `champ vide, séparateur inattendu ou nombre invalide`.

## Exemple corrigé précis
- Exemple guidé : `nom;age;ville avec trois enregistrements`.
- Correction guidée : lire l’en-tête, convertir les champs utiles, filtrer puis agréger, donc `liste de dictionnaires filtrée puis moyenne calculée`.
...
## Corrigé
- Corrigé exercice 1 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF1.
- Corrigé exercice 2 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF2.
- Corrigé exercice 3 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF3.
- Corrigé exercice 4 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF4.
- Corrigé exercice 5 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF1.
- Corrigé exercice 6 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF2.
- Corrigé exercice 7 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF3.
- Corrigé exercice 8 : donnée `table structurée en lignes et colonnes`, méthode `lire l’en-tête, convertir les champs utiles, filtrer puis agréger`, résultat `liste de dictionnaires filtrée puis moyenne calculée`, contrôle EF4.
