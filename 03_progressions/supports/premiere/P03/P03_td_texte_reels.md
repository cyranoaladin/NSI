...
### Exercice 9 — Représentation de nombres réels en binaire (P-DATA-BASE-03)

**Question 9a.** Convertir 0.75 en binaire par la méthode des multiplications successives. La représentation est-elle exacte ?

**Question 9b.** Convertir 0.1 en binaire (5 premières étapes). Le développement est-il fini ? Quel est le résultat de `0.1 + 0.2` en Python ?

**Question 9c.** Convertir 5.625 en binaire (partie entière + partie fractionnaire).

**Question 9d.** Expliquer pourquoi le test `0.1 + 0.2 == 0.3` renvoie `False` en Python. Proposer une méthode correcte pour comparer deux flottants.

### Corrigé exercice 9

**9a.** 0.75 × 2 = 1.5 → bit 1, reste 0.5. 0.5 × 2 = 1.0 → bit 1, reste 0. Résultat : 0.75₁₀ = 0.11₂. La représentation est **exacte** car 0.75 = 1/2 + 1/4, somme de puissances négatives de 2.

**9b.** 0.1 × 2 = 0.2 → 0 ; 0.2 × 2 = 0.4 → 0 ; 0.4 × 2 = 0.8 → 0 ; 0.8 × 2 = 1.6 → 1, reste 0.6 ; 0.6 × 2 = 1.2 → 1, reste 0.2. Le cycle se répète à l'infini : 0.1₁₀ = 0.00011001100…₂. En Python, `0.1 + 0.2` donne `0.30000000000000004`.

**9c.** Partie entière : 5 = 101₂. Partie fractionnaire : 0.625 × 2 = 1.25 → 1, reste 0.25 ; 0.25 × 2 = 0.5 → 0 ; 0.5 × 2 = 1.0 → 1. Résultat : 5.625₁₀ = 101.101₂. Exacte.

**9d.** `0.1` et `0.2` sont arrondis en machine ; leur somme n'est pas exactement 0.3. Méthode correcte : `abs(a - b) < 1e-9`.
